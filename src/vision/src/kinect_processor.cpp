...
vector<cv::Mat> templates(26); 
vector<char> corresponding(26); //indices indicate the letter that the template image is
...
int main(int argc, char **argv)
{
    ...
    // set chars from 'a'=97 to 'z'
    for (int i = 0; i < 26; i++) {

        corresponding[i] = 97 + i;

        string tmpPath = path + corresponding[i];
        tmpPath += ".png";

        cout << "loading template: " << tmpPath << endl;
        templates[i] = cv::imread(tmpPath, CV_LOAD_IMAGE_GRAYSCALE);
    }
